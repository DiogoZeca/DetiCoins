...
int main(int argc, char **argv)
{
  ...
  char *kernel_source = load_kernel_source("OpenCL/aad_sha1_opencl_kernel.cl");
  ...
  if(load_opencl_kernel(&ctx, kernel_source, "mine_deti_coins_v2", "-cl-fast-relaxed-math -cl-mad-enable") != 0)
  {
    free(kernel_source);
    cleanup_opencl(&ctx);
    return 1;
  ...
  printf("\n╔════════════════════════════════════════════════════════════╗\n");
  printf("║ OpenCL GPU FINAL STATISTICS                                ║\n");
  printf("╠════════════════════════════════════════════════════════════╣\n");
  printf("║ GPU: %-51s║\n", ctx.device_name);
